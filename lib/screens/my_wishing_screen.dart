...
class MyWishingScreen extends StatefulWidget {
  final List<BookModel> items;
  final String title;

  MyWishingScreen({super.key, required this.title, required this.items});

  @override
  State<MyWishingScreen> createState() => _MyWishingScreenState();
}

class _MyWishingScreenState extends State<MyWishingScreen> {
  ...
  Future<void> _deleteWishing(BookModel book) async {
    try {
      // 찜 정보 초기화
      final updatedBook = book.copyWith(isWishing: false);
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('찜한 책이 삭제되었습니다')));
      }
    } catch (e) {
      print('리뷰 삭제 오류: $e');
      if (mounted) {
